...
const Schema = z.object({
  ...
  promoCode: z.string().min(3).max(30).regex(/^[A-Za-z0-9]+$/).optional(),
  ...
  .handler(async ({ data }) => {
    ...
    if (data.promoCode) {
      const { data: promo } = await supabaseAdmin
        .from("promo_codes").select("*").ilike("code", data.promoCode).maybeSingle();
    ...
    let { data: customer } = await supabaseAdmin
      .from("customers").select("*").eq("email", data.email).maybeSingle();
    if (!customer) {
      const { data: created } = await supabaseAdmin.from("customers").insert({
        email: data.email, full_name: data.fullName ?? null, affiliate_id: affiliateId,
      }).select("*").single();
      customer = created;
    }
