...
export const Route = createFileRoute("/_authenticated/sam/managers")({
  component: SamManagersRoute,
});

function SamManagersRoute() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["sam-managers", user?.id],
    enabled: !!user,
    queryFn: async () => {
      // First get this SAM's own commission rate so we can calculate the cut.
      const { data: myProfile } = await supabase.from("profiles").select("commission_rate").eq("id", user!.id).single();
      const myRate = myProfile?.commission_rate || 0.01;
      ...
      if (!roles?.length) return { managers: [], myRate };
      ...
      // Only show children assigned to this SAM
      if (user) {
        query = query.eq("sam_id", user.id);
      }

      const { data: managersData } = await query;
      return { managers: managersData ?? [], myRate };
    },
  });

  const managers = data?.managers || [];
  const myRate = data?.myRate || 0;
  ...
  const handleAddSubmit = async () => {
    try {
      ...
      setAddOpen(false);
      qc.invalidateQueries({ queryKey: ["sam-managers"] });
    } catch (e: any) {
  ...
  const openAdd = () => {
    setForm({ id: "", firstName: "", lastName: "", email: "", password: "", commissionRate: 4, status: "active", phone: "" });
    setAddOpen(true);
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[13px] text-muted-foreground mb-6">Manage managers assigned to you</p>
      </div>
      <Card>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Your Commission (1%)</TableHead>
            ...
            <TableBody>
              {filteredManagers.map((mgr: any) => {
                const theirRate = mgr.commission_rate || 0.04;
                const earnedTotal = mgr.total_earned_cents || 0;
                // Proportional deduction based on revenue (approx cut map)
                const myCommissionCents = theirRate > 0 ? (earnedTotal / theirRate) * myRate : 0;
                return (
                  <TableRow key={mgr.id}>
                    ...
                    <TableCell>{mgr.commission_rate ? `${(mgr.commission_rate * 100).toFixed(1)}%` : "4%"}</TableCell>
                    <TableCell className="text-green-600 font-semibold">${(myCommissionCents / 100).toFixed(2)} <span>({(myRate * 100).toFixed(1)}%)</span></TableCell>
                    <TableCell>
              ...
              {filteredManagers.length === 0 && !isLoading && (
                <TableRow><TableCell colSpan={10} className="text-center text-muted-foreground py-4">No managers found.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </CardContent>
        ...
        <DialogContent className="sm:max-w-[520px]">
          ...
          <div className="grid grid-cols-2 gap-4 py-4">
            ...
            <div className="col-span-2 space-y-2">
              ...
              <Input type="tel" placeholder="(555) 123-4567" value={(form as any).phone || ""} onChange={e => setForm({ ...form, phone: e.target.value } as any)} />
            </div>
            <div className="col-span-2 space-y-2">
