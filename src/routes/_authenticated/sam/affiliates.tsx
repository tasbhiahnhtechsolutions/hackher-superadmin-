...
export const Route = createFileRoute("/_authenticated/sam/affiliates")({
  component: SamAffiliatesRoute,
});

function SamAffiliatesRoute() {
  ...
  const { data: managers } = useQuery({
    queryKey: ["sam-managers-list", user?.id],
    enabled: !!user,
    queryFn: async () => {
      ...
      if (user) query = query.eq("parent_user_id", user.id);
  ...
  const { data: affiliates, isLoading } = useQuery({
    queryKey: ["sam-affiliates", user?.id],
    enabled: !!user,
    queryFn: async () => {
      ...
      let query = supabase.from("affiliate_analytics_view" as any).select("*").in("id", userIds);
      if (user) query = query.eq("sam_id", user.id);
      const { data } = await query;
  ...
  const handleRemoveSocial = (index: number) => {
    const newHandles = [...form.socialHandles]; newHandles.splice(index, 1);
    setForm(f => ({ ...f, socialHandles: newHandles }));
  };
  const handleSocialChange = (index: number, val: string) => {
    const newHandles = [...form.socialHandles]; newHandles[index] = val;
    setForm(f => ({ ...f, socialHandles: newHandles }));
  };
  ...
  const filtered = useMemo(() => {
    ...
    return affiliates.filter((aff: any) => {
      ...
      const matchSearch = !q || (aff.full_name || "").toLowerCase().includes(q) || (aff.email || "").toLowerCase().includes(q);
      const matchStatus = statusFilter === "all" || (aff.status || "active") === statusFilter;
      const matchMgr = managerFilter === "all" || aff.manager_id === managerFilter;
      return matchSearch && matchStatus && matchMgr;
    });
  }, [affiliates, search, statusFilter, managerFilter]);

  const resetFilters = () => { setSearch(""); setStatusFilter("all"); setManagerFilter("all"); };

  const handleAddSubmit = async () => {
    try {
      setSubmitting(true);
      if (!form.assignedManager) throw new Error("Please assign a manager.");
      await createFn({
        data: {
          role: "affiliate",
          email: form.email,
          ...
          parentUserId: form.assignedManager,
      ...
      setAddOpen(false);
      qc.invalidateQueries({ queryKey: ["sam-affiliates"] });
    } catch (e: any) {
    ...
    try {
      ...
      await updateFn({
        data: {
          ...
          fullName: `${form.firstName} ${form.lastName}`.trim(),
          commissionRate: form.commissionRate / 100,
          parentUserId: form.assignedManager || undefined,
          phoneNumber: form.phone,
          socialHandles: form.socialHandles.filter(h => h.trim() !== ""),
          contractStart: form.contractStart,
          contractEnd: form.contractEnd,
      ...
      <Card>
        ...
        <div className="flex flex-wrap gap-2 px-6 py-3 border-t border-b bg-muted/30 items-center">
          <div className="relative flex-1 min-w-[180px] max-w-xs">
            ...
            <Input className="pl-8 h-8 text-sm" placeholder="Search affiliates..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
          <Select value={managerFilter} onValueChange={setManagerFilter}>
            <SelectTrigger className="h-8 w-[160px] text-sm"><SelectValue placeholder="All managers" /></SelectTrigger>
            <SelectContent>
      ...
      <Dialog open={addOpen} onOpenChange={setAddOpen}>
        <DialogContent className="sm:max-w-[620px] max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4 py-4">
            ...
            <div className="space-y-2">
              <Label>Email</Label>
              <Input type="email" placeholder="affiliate@example.com" value={form.email} onChange={e => setForm(prev => ({ ...prev, email: e.target.value }))} />
            </div>
            <div className="space-y-2">
              <Label>Password</Label>
              <Input type="password" placeholder="Leave blank to keep same" value={form.password} onChange={e => setForm(prev => ({ ...prev, password: e.target.value }))} />
            </div>
            <div className="col-span-2 space-y-2">
              <Label>Cell Phone Number</Label>
              <Input type="tel" placeholder="(555) 123-4567" value={form.phone} onChange={e => setForm(prev => ({ ...prev, phone: e.target.value }))} />
            </div>
            <div className="col-span-2 space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input type="date" value={form.contractStart} onChange={e => setForm(prev => ({ ...prev, contractStart: e.target.value }))} />
            </div>
            <div className="space-y-2">
              <Label>Contract Expiration Date</Label>
              <Input type="date" value={form.contractEnd} onChange={e => setForm(prev => ({ ...prev, contractEnd: e.target.value }))} />
            </div>
            <div className="col-span-2 space-y-2">
              <Label>Commission %</Label>
              <Input type="number" min="0" max="30" value={form.commissionRate} onChange={e => setForm(prev => ({ ...prev, commissionRate: Number(e.target.value) }))} />
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="sm:max-w-[620px] max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4 py-4">
            ...
            <div className="col-span-2 space-y-2">
              <Label>Assign to Manager</Label>
              <Select disabled value={form.assignedManager} onValueChange={v => setForm({ ...form, assignedManager: v })}>
                <SelectTrigger className="bg-muted cursor-not-allowed"><SelectValue placeholder="Select a manager" /></SelectTrigger>
                <SelectContent>
                  {managers?.map(m => (
                    <SelectItem key={m.id} value={m.id}>{m.full_name || m.email}</SelectItem>
                  ))}
            ...
            <div className="col-span-2 p-3 bg-[#FEF3C7] bg-opacity-[0.5] rounded-lg">
              <p className="text-xs text-[#92400E]"><strong>Reminder:</strong> To reassign this affiliate to another manager, contact the Super Admin Manager.</p>
            </div>
          </div>
          <DialogFooter>
