...
const FIELDS: Array<{ section: string; items: Array<{ key: string; label: string; desc?: string }> }> = [
  { section: "Email notifications", items: [
    ...
    { key: "email_subscription", label: "Subscription & billing", desc: "Receipts, failed payments, cancellations." },
    { key: "email_security", label: "Security", desc: "Password resets, login alerts." },
    { key: "email_admin_alerts", label: "Admin alerts", desc: "(Admins only) refunds, chargebacks, system events." },
    { key: "email_marketing", label: "Product updates", desc: "Occasional product news. Off by default." },
  ]},
...
function PrefsPage() {
  ...
  const update = async (key: string, value: boolean) => {
    ...
    const { error } = await supabase.from("notification_preferences").upsert({ user_id: user.id, [key]: value, updated_at: new Date().toISOString() } as never, { onConflict: "user_id" } as never);
          ...
          <CardContent className="space-y-4">
            {s.items.map((it) => (
              <div key={it.key} className="flex items-start justify-between gap-4">
                ...
                <Switch id={it.key} checked={prefs[it.key] ?? true} onCheckedChange={(v) => update(it.key, v)} />
              </div>
            ))}
          </CardContent>
        </Card>
