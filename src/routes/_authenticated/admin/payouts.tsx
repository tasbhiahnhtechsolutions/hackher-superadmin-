...
function Page() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["payouts"],
    queryFn: async () =>
      (await supabase.from("payouts").select("*").order("created_at", { ascending: false })).data ??
      [],
  });
  ...
  const generate = useMutation({
    ...
    onSuccess: (r) => {
      toast.success(
        `Generated ${r.created} payout(s) totaling $${(r.totalCents / 100).toFixed(2)}`,
      );
      setOpen(false);
  ...
  return (
    <>
      <PageHeader
        title="Payouts"
        subtitle="Generate payouts from cleared commissions"
        action={<Button onClick={() => setOpen(true)}>Generate payouts</Button>}
      />
      <PageBody>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground">
            Aggregates all cleared (un-paid) commissions in the selected window into a payout per
            beneficiary.
          </p>
