...
function EmailsPage() {
  ...
  const load = async () => {
    ...
    let q = supabase
      .from("email_send_log")
      .select(
        "id,template_name,recipient_email,status,subject,error_message,created_at,message_id,retry_count",
      )
  ...
  const onRetry = async (id: string) => {
    try {
      const r = await retryFn({ data: { logId: id } });
      r.ok ? toast.success("Resent") : toast.error(r.error || r.skipped || "Failed");
      load();
    } catch (e) {
  ...
  const onWorker = async () => {
    try {
      const r = await runFn();
      toast.success(`Retried ${r.retried} email(s)`);
      load();
    } catch (e) {
      toast.error((e as Error).message);
    }
  ...
  const onTest = async () => {
    if (!testEmail) return;
    try {
      const r = await testFn({ data: { to: testEmail } });
      r.ok ? toast.success("Test sent") : toast.error(r.error || "Failed");
      load();
    } catch (e) {
      toast.error((e as Error).message);
    }
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader
        ...
        subtitle="Audit log of every transactional email."
        action={
          <Button onClick={onWorker} variant="outline" size="sm">
            <RefreshCw className="mr-2 h-4 w-4" /> Run retry worker
          </Button>
        }
      />
                ...
                rows.map((r) => (
                  <TableRow key={r.id}>
                    ...
                    <TableCell>
                      {badge(r.status)}
                      {r.error_message && (
                        <div className="text-[11px] text-destructive mt-1 line-clamp-1">
                          {r.error_message}
                        </div>
                      )}
                    </TableCell>
                    ...
                    <TableCell className="text-right">
                      {r.status === "failed" && (
                        <Button variant="ghost" size="sm" onClick={() => onRetry(r.id)}>
                          Retry
                        </Button>
                      )}
                    </TableCell>
