...
function FraudPage() {
  ...
  const [filter, setFilter] = useState<"open" | "reviewing" | "dismissed" | "confirmed" | "all">(
    "open",
  );
            ...
            <TableBody>
              ...
              {rows.map((r) => (
                ...
                >
                  ...
                  <TableCell>
                    <Badge className={SEV_COLORS[r.severity] ?? ""} variant="secondary">
                      {r.severity}
                    </Badge>
                  </TableCell>
      ...
      <Dialog open={!!selected} onOpenChange={(o) => !o && setSelected(null)}>
        <DialogContent className="max-w-lg">
          ...
          {selected && (
            <div className="space-y-3 text-sm">
              <div className="flex gap-4 text-xs text-muted-foreground">
                <span>
                  Severity: <strong className="text-foreground">{selected.severity}</strong>
                </span>
                <span>
                  Risk score: <strong className="text-foreground">{selected.risk_score}</strong>
                </span>
                {selected.ip_address && (
                  <span>
                    IP: <strong className="text-foreground">{selected.ip_address}</strong>
                  </span>
                )}
              </div>
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => act("reviewing")}>
              Mark reviewing
            </Button>
            <Button variant="outline" onClick={() => act("dismissed")}>
              Dismiss
            </Button>
            <Button variant="destructive" onClick={() => act("confirmed")}>
              Confirm fraud
            </Button>
          </DialogFooter>
        </DialogContent>
