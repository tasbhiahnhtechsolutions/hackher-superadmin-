...
type Row = {
  id: string; created_at: string; amount_cents: number; rate: number;
  status: string; cleared_at: string | null;
...
function Page() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<Row[]> => {
      const { data: comm } = await supabase.from("commissions")
        .select("id,created_at,amount_cents,rate,status,cleared_at,subscription_id")
        .eq("beneficiary_id", user!.id).order("created_at", { ascending: false });
      const subIds = Array.from(new Set((comm ?? []).map((c) => c.subscription_id)));
      if (!subIds.length) return [];
      const { data: subs } = await supabase.from("subscriptions").select("id,promo_code_id").in("id", subIds);
      ...
      const subToCampaign = new Map((subs ?? []).map((s) => [s.id, s.promo_code_id ? promoToCampaign.get(s.promo_code_id) ?? null : null]));
      return (comm ?? []).map((c) => ({ ...c, campaign: subToCampaign.get(c.subscription_id) ?? null })) as Row[];
    },
  });
    ...
    <PageBody>
      ...
      <div className="rounded-xl border border-border/60 bg-card"><Table>
        <TableHeader><TableRow>
          <TableHead>Date</TableHead><TableHead>Campaign</TableHead><TableHead>Amount</TableHead>
          <TableHead>Rate</TableHead><TableHead>Status</TableHead><TableHead>Cleared</TableHead>
        </TableRow></TableHeader>
        <TableBody>
          {isLoading ? <TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">Loading…</TableCell></TableRow>
            : !filtered.length ? <TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">No earnings yet.</TableCell></TableRow>
