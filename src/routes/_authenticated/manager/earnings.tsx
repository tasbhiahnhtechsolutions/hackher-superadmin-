...
export const Route = createFileRoute("/_authenticated/manager/earnings")({
  component: ManagerEarningsRoute,
});
...
function ManagerEarningsRoute() {
  const { user } = useAuth();

  // Summary from manager_analytics_view
  const { data: summary } = useQuery({
    queryKey: ["manager-earnings-summary", user?.id],
    enabled: !!user,
    queryFn: async () => {
  ...
  const { data: holdData } = useQuery({
    queryKey: ["manager-earnings-hold", user?.id],
    enabled: !!user,
    queryFn: async () => {
      ...
      const subs = new Set((data ?? []).map((r: any) => r.subscriber_id).filter(Boolean)).size;
      return { total, subs };
    },
  });
  ...
  const { data: monthlyPayouts, isLoading } = useQuery({
    queryKey: ["manager-earnings-monthly", user?.id],
    enabled: !!user,
    queryFn: async () => {
      const { data } = await supabase
        .from("commissions")
  ...
  const metrics = [
    {
      ...
      sub: "Cumulative 4% manager commission",
      icon: DollarSign,
    ...
    },
    {
      label: "Pending",
      value: fmt(pending),
      sub: "Cleared from hold, awaiting payment",
      icon: Clock,
    ...
    },
    {
      label: "30-Day Hold",
      value: fmt(hold),
      sub: `${holdSubs} subscriber${holdSubs !== 1 ? "s" : ""} in chargeback window`,
      icon: ShieldAlert,
      color: "text-rose-500",
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[13px] text-muted-foreground mb-6">
          Your manager-tier (4%) commission earnings · Net 60 payout cycle
        </p>
      </div>
        ...
        <CardHeader>
          <div>
            ...
            <p className="text-[12px] text-muted-foreground mt-1">
              Read-only — your SAM manages your payouts.
            </p>
          </div>
        </CardHeader>
        <CardContent>
          <Table>
            ...
            <TableBody>
              ...
              {!isLoading && monthlyPayouts?.length === 0 && (
                <TableRow>
                  <TableCell colSpan={3} className="text-center text-muted-foreground py-6">No earnings records found.</TableCell>
                </TableRow>
              )}
