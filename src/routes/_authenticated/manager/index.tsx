  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const [aff, comm] = await Promise.all([
        supabase.from("profiles").select("id", { count: "exact", head: true }).eq("parent_user_id", user!.id),
  ...
  return (
    <RoleDashboard
      ...
      kpis={[
        ...
        { label: "Cleared Earnings", value: `$${((data?.cleared ?? 0) / 100).toFixed(2)}`, tone: "success" },
      ]}
    />
  );
}
