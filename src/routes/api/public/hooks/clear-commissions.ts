    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        if (due?.length) {
          ...
          for (const c of due) totals.set(c.beneficiary_id, (totals.get(c.beneficiary_id) ?? 0) + c.amount_cents);
          for (const [beneficiaryId, amount] of totals) {
            const { data: prof } = await supabaseAdmin.from("profiles").select("email").eq("id", beneficiaryId).maybeSingle();
            await supabaseAdmin.rpc("notify_user_with_pref" as never, {
              _user_id: beneficiaryId, _category: "commissions", _type: "commission_cleared",
            ...
            if (prof?.email) {
              await sendAppEmail({
                to: prof.email, template: "commission_cleared",
                data: { amountCents: amount, currency: "usd", dashboardUrl: `${APP_URL}/affiliate/earnings` },
                category: "commissions", userId: beneficiaryId,
