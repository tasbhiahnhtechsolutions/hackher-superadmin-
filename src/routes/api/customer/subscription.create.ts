...
// Internal Customer Portal endpoint — creates Stripe checkout session with affiliate attribution.
// Body: { email, full_name?, planId, coupon?, success_url, cancel_url }
...
const Schema = z.object({
  ...
  coupon: z.string().min(3).max(30).regex(/^[A-Za-z0-9]+$/).optional(),
  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        if (!plan.stripe_price_id) return jsonError(409, "plan_not_synced");
        ...
        if (input.coupon) {
          ...
          if (promo && promo.status === "active" && (!promo.usage_limit || promo.usage_count < promo.usage_limit)) {
            ...
            return jsonError(400, "invalid_coupon");
          }
        }
