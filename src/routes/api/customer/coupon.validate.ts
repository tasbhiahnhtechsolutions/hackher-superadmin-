    ...
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        if (promo.affiliate_id) {
          const { data: aff } = await supabaseAdmin.from("profiles").select("full_name").eq("id", promo.affiliate_id).maybeSingle();
        ...
        if (parsed.data.planId) {
          const { data: plan } = await supabaseAdmin.from("plans").select("price_cents,currency").eq("id", parsed.data.planId).maybeSingle();
          if (plan) {
            originalPriceCents = plan.price_cents;
            currency = plan.currency;
            finalPriceCents = Math.round(plan.price_cents * (1 - Number(promo.discount_percent) / 100));
          }
        }
