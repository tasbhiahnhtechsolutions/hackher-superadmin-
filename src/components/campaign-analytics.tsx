...
export function CampaignAnalytics({
  ...
  const perf = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.rpc(
        "report_campaign_performance" as never,
        {
          _scope_user: user!.id,
          _start: range.start,
          _end: range.end,
  ...
  const series = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.rpc(
        "report_campaign_timeseries" as never,
        {
          _scope_user: user!.id,
          _start: range.start,
          _end: range.end,
  ...
  const top = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.rpc(
        "report_top_promo_codes" as never,
        {
          _scope_user: user!.id,
          _start: range.start,
          _end: range.end,
  ...
  const planMix = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.rpc(
        "report_plan_conversion_by_campaign" as never,
        {
          _scope_user: user!.id,
          _start: range.start,
          _end: range.end,
  ...
  const campaigns = useMemo(
    () => Array.from(new Set((series.data ?? []).map((r) => r.campaign))).slice(0, 5),
    [series.data],
  );
  const trendChart = useMemo(() => {
    const byBucket = new Map<string, Record<string, number | string>>();
    for (const r of series.data ?? []) {
      const k = new Date(r.bucket).toLocaleDateString(undefined, {
        month: "short",
        day: "numeric",
      });
      const cur = byBucket.get(k) ?? { date: k };
      cur[r.campaign] =
        (Number(cur[r.campaign] ?? 0) as number) + Number(r.gross_revenue_cents) / 100;
      byBucket.set(k, cur);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm font-medium">Revenue by campaign · over time</CardTitle>
        </CardHeader>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="text-sm font-medium">Plan mix by campaign</CardTitle>
          </CardHeader>
          <CardContent className="h-72">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm font-medium">Top promo codes</CardTitle>
          </CardHeader>
              ...
              <TableBody>
                {(top.data ?? []).map((r) => (
                  <TableRow key={r.code}>
                    ...
                    <TableCell>
                      <Badge variant="secondary">{r.campaign}</Badge>
                    </TableCell>
                    <TableCell className="text-right">{r.subscriptions}</TableCell>
                    <TableCell className="text-right font-medium">
                      {fmt$(r.gross_revenue_cents)}
                    </TableCell>
                  </TableRow>
                ))}
              ...
              {(perf.data ?? []).map((r) => (
                <TableRow key={r.campaign}>
                  ...
                  <TableCell className="text-right">
                    {(Number(r.conversion_rate) * 100).toFixed(1)}%
                  </TableCell>
